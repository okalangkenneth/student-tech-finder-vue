...
// We keep budget for backward compatibility with the API.
// (Server prefers minPrice/maxPrice when provided.)
...
const ctaText = computed(() => `Find ${topicLabels[topic.value] ?? 'items'}`)
...
async function doSearch (reset = false) {
  ...
  try {
    ...
    if (data?.ok) {
      ...
      // merge + de-dupe by title/brand/price
      const incoming = (data.results || []) as Product[]
      const merged = reset ? incoming : items.value.concat(incoming)
...
async function watchPrice (p: Product) {
...
<template>
  <main class="wrap">
    ...
    <!-- Price range + broaden + CTA -->
    <div class="filters">
      <label class="field">
        <span>Min ($)</span>
        <input type="number" min="0" step="10" v-model.number="minPrice" placeholder="0" inputmode="numeric" />
      </label>

      <label class="field">
        <span>Max ($)</span>
        <input type="number" min="0" step="10" v-model.number="maxPrice" placeholder="900" inputmode="numeric" />
      </label>
    ...
    <div v-else class="grid">
      <!-- SINGLE v-for (fixed). The previous nested v-for caused duplicates. -->
...
.btn.primary{ background:#22c55e; color:#0c0c0c; }
...
.tiny{ color:#9aa3af; font-size:.85rem; }
</style>
